...
function CouponPopover({ 
  ...
  const { data: availableCoupons, isLoading: isLoadingCoupons } = useGetAvailableCouponsQuery();
  ...
  useEffect(() => {
    if (selectedCoupons) {
      setLocalSelectedCoupons(selectedCoupons);
  ...
  const handleCouponClick = (coupon) => {
    try {
      ...
      // If the same coupon is clicked again, toggle it off
      if (localSelectedCoupons[type] && localSelectedCoupons[type]._id === coupon._id) {
        setLocalSelectedCoupons(prev => ({
          ...prev,
          [type]: null
        }));
        ...
        // Otherwise select the new coupon for this category
        setLocalSelectedCoupons(prev => ({
          ...prev,
          [type]: coupon
        }));
  ...
  const handleDone = () => {
    if (setSelectedCoupons) {
      setSelectedCoupons(localSelectedCoupons);
    }
    
    // For backward compatibility
    const selectedCouponArray = Object.values(localSelectedCoupons).filter(Boolean);
    setSelectedCoupon(selectedCouponArray.length > 0 ? selectedCouponArray[0] : null);
    
    onClose();
  };

  // Group coupons by target_type
  const groupedCoupons = useMemo(() => {
    ...
    availableCoupons.forEach(coupon => {
      if (grouped[coupon.target_type]) {
        grouped[coupon.target_type].push(coupon);
      } else {
        grouped.ORDER.push(coupon);
      }
    });
  ...
  const filteredGroupedCoupons = useMemo(() => {
    ...
    return filtered;
  }, [groupedCoupons, availableCoupons]);
  ...
  const getCouponValueText = (coupon) => {
    if (coupon.type === "PERCENT") {
      return `${coupon.value}% (tối đa ${formatCurrency(coupon.max_value)})`;
    } else {
  ...
  // Get background color based on target_type
  const getCouponTypeColor = (type) => {
    switch (type) {
      case 'PRODUCT':
        return {
          light: '#FFF0F7', // Light pink
          main: '#FFB6C1', // Pink
          dark: '#FF69B4', // Hot pink
          text: '#D81B60' // Dark pink for text
        };
      case 'FREESHIPPING':
        return {
          ...
          text: '#1565C0' // Darker blue for text
        };
      case 'ORDER':
      default:
        return {
          light: '#FFFDE7', // Light yellow
          main: '#FFF59D', // Yellow
          dark: '#FBC02D', // Dark yellow
          text: '#F57F17' // Amber for text
        };
    }
  };

  // Get title for each coupon type section
  ...
  return (
    <Popover
      ...
      anchorPosition={{ top: window.innerHeight / 2 - 300, left: window.innerWidth / 2  }}
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" fontWeight="500">
              Mã giảm giá có sẵn ({Object.values(filteredGroupedCoupons).reduce((acc, coupons) => acc + coupons.length, 0)})
            </Typography>
          </Box>
          ...
          >
            {isLoadingCoupons ? (
              ...
              <>
                      ...
                      {coupons.map((coupon) => (
                        <Box
                          ...
                          sx={{
                            ...
                            border: "1px solid",
                            borderColor:
                              localSelectedCoupons[type] && localSelectedCoupons[type]._id === coupon._id
                                ? colors.dark
                                : "divider",
                            bgcolor:
                              localSelectedCoupons[type] && localSelectedCoupons[type]._id === coupon._id
                        ...
                        >
                          ...
                          <Typography variant="caption" color="text.secondary">
                            HSD: {new Date(coupon.end_day).toLocaleDateString('vi-VN')}
                          </Typography>
                        </Box>
                      ))}
                    </Box>
                ...
                {Object.values(filteredGroupedCoupons).every(coupons => coupons.length === 0) && (
                  <Box sx={{ p: 2, textAlign: 'center' }}>
                    <Typography color="text.secondary">Không tìm thấy mã giảm giá</Typography>
                  </Box>
                )}
              </>
            )}
          ...
          <Box sx={{ mt: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              ...
              {Object.values(localSelectedCoupons).every(coupon => !coupon) && (
